...
- 리스트가 아래로 스크롤 시 상단으로 이동하기 버튼 구현
- RecyclerView 내부에 fragment issue -> '<include>'방식으로 해결 -> 터치 시 중복발생 문제점 존재
- 상단으로 이동하기 버튼 클릭 시 리스트 상단으로 이동
- 리스트 내용 선택 시 해당 뉴스의 id값과 title을 가져오는 기능 구현
- 사용자가 키워드리스트에서 키워드 선택 시 메인뉴스 리스트에서 해당 키워드가 존재하는 위치로 이동하는 기능 구현
* 
